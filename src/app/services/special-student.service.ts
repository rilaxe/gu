...
export class SpecialResultService {
    genk: GenericService;
  userName: string = '';
  userImgPath: string = '';
  adminStatus: string;
  ...
  createResult(id, term, currentSessionId, currentSession, classId, average, position, examtype) {
    this.studentId = id;
  ...
  getResultsByStudent() {
    Swal.fire({
      ...
      didOpen: () => {
        ...
        this.sch.getSpecialResultsByStudent(this.currentClassId, this.currentSessionId.toString(), this.currentTerm, this.studentId, this.examType)
          .subscribe(res => {
            this.resultlist = res;
            Swal.close();
            this.getSchool();
  ...
  public async convertToPDF() {
    ...
    doc.setFont('', '', '700');
    doc.text('POSITION IN CLASS:', 380, 156);
    doc.setFont('', '', '500');
    doc.text(this.position.toString(), 470, 156);

//     doc.setFont('', '', '700');
//     doc.text('NEXT TERM BEGINS:', );
//     doc.setFont('', '', '500');
//     doc.text('', 460, 156);
...
//   }
    ...
    doc.setFontSize(12);
    doc.setFont('Trebuchet MS', 'normal', '600');
    doc.text(this.examType + ' RESULT ' + 'REPORT SHEET',pageWidth / 2, 190, {align: 'center'});
...
autoTable(doc, {
  ...
  startY: customersTrailTableEndY + 10, margin: { top: 150, left: 20, bottom: 30 },

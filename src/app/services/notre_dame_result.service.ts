...
export class NotreDameResultService {
  @Input() header: string;
  genk: GenericService;
  userName: string = '';
  userImgPath: string = '';
...
getSchoolPerformanceAnalysis() {
                ...
                didOpen: async () => {
                    ...
                    const interval = setInterval(() => {
                        //if (this.genk.logoString && this.resultlist.length > 0 && this.genk.signatureString && this.genk.photoString) {
                            clearInterval(interval);
                            clearTimeout(timeout); // Stop checking once the condition is met
                            this.renderPdfChart(this.mychart, this.mychartdiv); // Call the desired function
                            Swal.close();
                        //}
                    }, 500);
  ...
  // }
    ...
    getResultsByStudentPerformanceAnalysis() {
        Swal.fire({
            title: 'Loading Result....',
            allowEscapeKey: false,
            allowOutsideClick: false,
            ...
            didOpen: () => {
                ...
                this.sch.getResultsByStudent(this.currentClassId, this.currentSessionId.toString(), this.currentTerm, this.studentId)
                    .subscribe(res => {
                        ...
                        // let kool = averageList.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
                        Swal.close();
                        this.getSchoolPerformanceAnalysis();
                    })
            }
        });
    }
...
renderPdfChart(myChart: HTMLCanvasElement, mychartdiv: HTMLDivElement) {
  ...
  const myPieChart = new Chart(ctx, {
    ...
    doc.setFont('', '', '700');
    //let me = doc.splitTextToSize(this.myschool?.address ?? '', 220);
    doc.text('TERMLY EVALUATION ANALYSIS FOR ' + ((this.studentobj?.surname ?? '') + ' ' + (this.studentobj?.middleName ?? '') + ' ' + (this.studentobj?.firstName ?? '')).trim(), pageWidth / 2, 85, {align: 'center'});
...
const parentDiv = mychartdiv;
parentDiv.removeChild(myChart);

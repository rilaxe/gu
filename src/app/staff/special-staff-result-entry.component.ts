...
@Component({
  ...
  templateUrl: './special-staff-result-entry.component.html',
  styleUrls: ['staff-result.component.css']
})
export class SpecialStaffResultEntryComponent implements OnInit {
  @Input() header: string;
...
ngOnInit(): void {
  this.genk.topdata = 'Special Result Entry';
  this.resultForm = new FormGroup({
      'term': new FormControl(this.currentTerm, [Validators.required]),
      'examtype': new FormControl(this.examType, [Validators.required]),
...
getClassNames() {
  ...
  getResultsByClass() {
    this.sch.getSpecialResultsByClass(this.currentClass.toString(), this.currentSessionId.toString(), this.currentTerm, this.currentSubjectId.toString(), this.examType)
      .subscribe(res => {
        this.resultlist = res;
        this.bigboss = [];
      })
  ...
  getCaSetupByClass() {
    this.sch.getSpecialCaSetupByClass(this.currentSessionId.toString(), this.currentTerm, this.examType)
    .subscribe(res => {
  ...
  setCa(studentId: number, ca: HTMLInputElement, key: string) {
    ...
    if (this.bigboss.filter(t => t.studentId == Number(studentId)).length > 0) {
      ...
      let caValue = this.clearInput(this.caScoreData[key.toUpperCase()], Number(ca.value));
      ...
      Number(newResult['ca3'] ?? this.checkCA(oldResult.ca3)) + Number(newResult['exam'] ?? this.checkCA(oldResult.exam));
  ...
  saveResultEntry() {
    ...
    Swal.fire({
      ...
      didOpen: () => {
        Swal.showLoading();
    this.sch.saveSpecialResultEntry(this.bigboss, this.currentSessionId.toString(), this.currentTerm, this.currentSubjectId.toString(), this.currentClass.toString(), className, this.examType)
  ...
  clearInput(max: number, value: number) {
    if (max) {
      if(value > max) {
        alert('Value entered is greater than CA Maximum Score of ' + max);
        return this.initvalue ? Number(this.initvalue) : 0;
      } else {
        return Number(value);
      }
    } 

...
@Component({
  ...
  templateUrl: './staff-result-entry.component.html',
  styleUrls: ['staff-result.component.css']
})
export class StaffResultEntryComponent implements OnInit {
  @Input() header: string;
...
getClassNames() {
  ...
  getResultsByClass() {
    this.sch.getResultsByClass(this.currentClass.toString(), this.currentSessionId.toString(), this.currentTerm, this.currentSubjectId.toString())
      .subscribe(res => {
        this.resultlist = res;
        this.bigboss = [];
      })
  ...
  validateSaveEntry() {
    let mysubject = this.classSubjects.filter(a => a.subjectId == this.currentSubjectId)[0]?.subjects;

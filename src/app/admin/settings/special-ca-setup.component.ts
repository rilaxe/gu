...
@Component({
  ...
  templateUrl: './special-ca-setup.component.html',
  styleUrls: ['./settings.component.css']
})
export class SpecialCASetupComponent implements OnInit {
  @Input() header: string;
  genk: GenericService;
  userName: string = '';
  userImgPath: string = '';
...
ngOnInit(): void {
  this.genk.topdata = 'Special CA Setup';
  this.levelCategory = 'REGULAR';
  this.getCaSetup();
  //this.getCaSetup();
}

getCaSetup() {
  this.sch.getSpecialCASetup(this.levelCategory)
  .subscribe(res => {
    this.caData = res;
...
checkIsEmpty(ca: CaSetup[]) {
  ...
  saveData() {
    ...
    if (this.levelCategory) {
      ...
      this.sch.saveSpecialCaSetup(newlist, this.levelCategory)

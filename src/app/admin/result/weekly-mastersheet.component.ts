...
@Component({
  templateUrl: './weekly-mastersheet.component.html',
  styleUrls: []
})
export class WeeklyMasterSheetComponent implements OnInit {
  @Input() header: string;
  genk: GenericService;
  userName: string = '';
  userImgPath: string = '';
...
ngOnInit(): void {
  this.genk.topdata = 'Weekly Mastersheet';
  debugger;
  this.getClassNames();
  this.getSession();
...
getClassNames() {
  ...
  getMastersheetByClass() {
    if (this.selectedWeek == 'ALLWEEKS') {
      this.sch.getWeeklyAnnualMastersheetByClassAllWeek(this.currentClass.toString(), this.currentSessionId.toString(), this.currentTerm)
      .subscribe(res => {
        this.resultlist = res;
      ...
      this.sch.getWeeklyMastersheetByClassOneSubject(this.currentClass.toString(), this.currentSessionId.toString(), this.currentTerm, this.selectedWeek.toString())
      .subscribe(res => {
        this.resultlist = res;
        let list = this.resultlist.sort((a, b) => b.count - a.count);
        this.subjectlist = list[0].subjects;
  ...
  changeWeek(value) {
    debugger
    this.selectedWeek = value ? value.toString() : null;
  }

  changeTerm(value) {
    debugger
    this.currentTerm = value ? value : null;
  }


  // this.sch.getAnnualMastersheetByClass(this.currentClass.toString(), this.currentSessionId.toString(), this.currentTerm)
  //     .subscribe(res => {
  //       this.resultlist = res;
  //       let list = this.resultlist.sort((a, b) => b.count - a.count);
  //       this.subjectlist = list[0].subjects;
  ...
  toExcel() {
    ...
    //this.customizeExcel(worksheet);
    const colName = ['C3', 'D3', 'E3', 'F3', 'G3', 'H3', 'I3', 'J3', 'K3', 'L3', 'M3', 'N3', 'O3', 'P3', 'Q3', 'R3', 'S3', 'T3', 'U3', 'V3'];
    debugger;

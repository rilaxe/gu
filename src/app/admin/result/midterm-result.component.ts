...
@Component({
  templateUrl: './midterm-result.component.html',
  styleUrls: ['../../table.css']
})
export class MidtermResultComponent implements OnInit {
  @Input() header: string;
  genk: GenericService;
  userName: string = '';
  userImgPath: string = '';
  ...
  public convertToPDFLouis(table: HTMLTableElement) {
    ...
    resu.forEach(c => {
      ...
      resArray.push(Number(c.totalScore?.toFixed(2)));
      resArray.push(this.calMinGrade(Number(c.totalScore?.toFixed(1)))?.grade);
